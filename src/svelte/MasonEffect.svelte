<script lang="ts">
  ...
  onMount(() => {
    ...
    instance = new MasonEffect(container, options);
  });
  ...
  export function morph(textOrOptions?: string | Partial<MasonEffectOptions>) {
    if (instance) {
  ...
  export function updateConfig(config: Partial<MasonEffectOptions>) {
    if (instance) {
